...
def training_epoch(train_data, model, optimizer, epoch, device):
    epoch_loss = []
    model.train()
    current = 0
    ...
    for batch in train_data:
        # prepare data
        inputs = {
            'ego_state': batch[0].to(device),
            'neighbors_state': batch[1].to(device),
            'map_lanes': batch[2].to(device),
            'map_crosswalks': batch[3].to(device),
        }

        ego_future = batch[4].to(device)
        neighbor_future = batch[5].to(device)
        ...
        logging.info(
            f"\rEpoch {epoch+1}|"+
            f"Progress: [{current:>6d}/{size*args.batch_size:>6d}]|"+
...
def validation_epoch(valid_data, model, epoch, device):
...
def main():
    ...
    if args.load_dir != '':
        ...
        model_ckpts = torch.load(model_path, map_location=device)
    ...
    train_data = DataLoader(
        ...
        shuffle=True, num_workers=0  # Set num_workers to 0 to disable multiprocessing
        )
    valid_data = DataLoader(
        valid_dataset, batch_size=args.batch_size,
        shuffle=False, num_workers=0  # Set num_workers to 0 to disable multiprocessing
        )
    ...
    for epoch in range(epochs):
        ...
        train_loss = training_epoch(train_data, model, optimizer, epoch, device)
        valid_metrics, val_loss = validation_epoch(valid_data, model, epoch, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device', type=str, help='device to use (cuda/cpu)', default='cuda')
    # data & loggings
